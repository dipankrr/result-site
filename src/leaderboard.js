...
export function updateLeaderboard(student, leaderboardData) {
    ...
    for (let rank = 1; rank <= 10; rank++) {
        ...
        const isCurrentStudent = students.some(s => s.roll === student.ROLL);
            ...
            ${students.length > 0 ? `
                <div class="rank-students" style="display: none;">
                    <div class="student-list">
                        ${students.map(s => `
                            <div class="student-item ${s.roll === student.ROLL ? 'current' : ''}">
                                <div class="student-name">
                                    ${s.name}
                                    ${s.roll === student.ROLL ? '<span class="you-badge">(You)</span>' : ''}
                                </div>
